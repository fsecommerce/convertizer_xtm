...
if (xtc_not_null($action)) {
  ...
  switch ($action) {
    ...
    case 'buy_now':
	  ...
	  /* BOE Convertizer - add product to cart via URL */
	  
	   if (isset($_GET['BUYproducts_sku'])) {
        $_GET['BUYproducts_sku'] = (int)$_GET['BUYproducts_sku'];
		
		/* get the products id by its sku */
		
		$sku = $_GET['BUYproducts_sku'];
		$query = xtc_db_query("SELECT products_id FROM products where products_model='" . mysql_real_escape_string($sku) . "'");
		$prod_data = xtc_db_fetch_array($query);
		$prod_id = $prod_data['products_id'];
        ...
        // check for FSK18
        if ($permission['products_fsk18'] == '1' && $_SESSION['customers_status']['customers_fsk18'] == '1') {
          xtc_redirect(xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .$prod_id, 'NONSSL'));
        }
        if ($_SESSION['customers_status']['customers_fsk18_display'] == '0' && $permission['products_fsk18'] == '1') {
          xtc_redirect(xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .$prod_id, 'NONSSL'));
        }
        ...
        if (GROUP_CHECK == 'true') {
          if ($permission['customer_group'] != '1') {
            xtc_redirect(xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .$prod_id));
          }
        }
        if (xtc_has_product_attributes($prod_id)) {
          xtc_redirect(xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .$prod_id));
        } else {
          if (isset ($_SESSION['cart'])) {
            $cart_quantity = ($_SESSION['cart']->get_quantity(xtc_get_uprid($prod_id,''))+1);
            if ($cart_quantity > MAX_PRODUCTS_QTY) {
              $cart_quantity = MAX_PRODUCTS_QTY;
            }
            if (isset($econda) && is_object($econda)) {
              $econda->_emptyCart();
              $old_quantity = $_SESSION['cart']->get_quantity($prod_id);
              $econda->_addProduct($prod_id, $cart_quantity, $old_quantity);
            }
            $_SESSION['cart']->add_cart($prod_id, $cart_quantity);
          } else {
            xtc_redirect(xtc_href_link(FILENAME_DEFAULT));
          }
        }
	  ...
	  /* EOE Convertizer - add product to cart via URL */
